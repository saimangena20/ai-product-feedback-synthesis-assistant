...
def run_analysis(db: Session, ingest_id: str, job_id: str | None = None) -> AnalysisJob:
    ...
    try:
        ...
        with db.begin():
            for suggestion in suggestions.themes:
                theme = Theme(id=str(uuid4()), ingest_id=ingest_id, name=suggestion.suggested_theme_label, description=suggestion.suggested_problem_statement, ai_suggested=True, review_status="suggested", advisory_confidence=suggestion.advisory_confidence, historical_match_id=suggestion.historical_match_id, historical_commentary=suggestion.historical_commentary, historical_similarity_score=suggestion.similarity_score)
                db.add(theme)
